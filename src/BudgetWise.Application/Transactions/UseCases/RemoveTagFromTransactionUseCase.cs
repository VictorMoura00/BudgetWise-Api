...
public sealed class RemoveTagFromTransactionUseCase(
    ...
    IUnitOfWork unitOfWork) : IUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!await transactionRepository.IsTagLinkedAsync(transactionId, tagId, cancellationToken))
            return Result.Failure(TagErrors.NotLinked);

        await transactionRepository.RemoveTagAsync(transactionId, tagId, cancellationToken);
